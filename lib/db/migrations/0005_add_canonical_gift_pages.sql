...
BEGIN
  ...
  IF NEW.slug IS NULL OR NEW.slug = '' THEN
    ...
    IF EXISTS (SELECT 1 FROM products WHERE slug = base_slug AND id <> NEW.id) THEN
      NEW.slug := LEFT(base_slug, 150) || '-' || LEFT(NEW.public_id::text, 8);
    ELSE
      NEW.slug := base_slug;
    END IF;
  END IF;
...
WHERE id = 'B0F9DZMQBL';--> statement-breakpoint
...
WHERE id = 'B005UGWDAE';--> statement-breakpoint
...
WHERE id = 'B0D57DDDM1';--> statement-breakpoint
